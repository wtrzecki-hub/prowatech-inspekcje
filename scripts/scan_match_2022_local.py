#!/usr/bin/env python3
"""Phase A migracji 2022 — lokalny scan Drive Stream G: + match po ew_designation.

Strategia (jak Faza 15.G dla 2024):
  - Pobierz placeholdery 2022 z bazy (304 wpisy, 301 pustych)
  - Pobierz wszystkie 425 turbin z ew_designation, turbine_code, client_name, farm_name
  - Skanuj GDrive `04 Inspekcje/2022/` rekurencyjnie
  - Per plik: filtr non-protocol, wyciągnij ew_designation pasujący w nazwie, match per klient
  - Output: manifest_2022_full_dryrun.json + manifest_2022_full.json (gotowy do upload)
...
Uruchomienie:
  python scripts/scan_match_2022_local.py            # dryrun
  python scripts/scan_match_2022_local.py --copy     # + kopia plików do scripts/output/pdfs/
"""
...
GDRIVE_2022 = Path(r"G:\Dyski współdzielone\21 PROWATECH - INSPEKCJE\04 Inspekcje\2022")
ROOT = Path(__file__).parent.parent
PDF_OUT_DIR = ROOT / "scripts/output/pdfs"
DRYRUN_PATH = ROOT / "scripts/output/manifest_2022_full_dryrun.json"
MANIFEST_PATH = ROOT / "scripts/output/manifest_2022_full.json"
...
PROTOCOL_NUM_RE = re.compile(r"^(\d+)_T_2022", re.IGNORECASE)
DATE_RE = re.compile(r"(\d{2})-(\d{2})-(2022)")
...
def fetch_db_data():
    ...
    while True:
        ...
        if len(res.data) < 1000:
            break
        offset += 1000
    print(f"[db] placeholdery 2022: {len(placeholders)} (puste: {sum(1 for p in placeholders if not p['protocol_pdf_url'])})")
...
def main():
    ...
    if not GDRIVE_2022.exists():
        print(f"[FATAL] Brak Drive Stream: {GDRIVE_2022}")
        sys.exit(2)
    ...
    # Skan plików
    all_pdfs = []
    for dirpath, dirnames, filenames in os.walk(str(GDRIVE_2022)):
        for fn in filenames:
            if fn.lower().endswith(".pdf"):
                all_pdfs.append(Path(dirpath) / fn)
    print(f"[scan] znaleziono {len(all_pdfs)} plików PDF w {GDRIVE_2022}")
    ...
    for pdf in all_pdfs:
        name = pdf.name
        try:
            rel_path = str(pdf.relative_to(GDRIVE_2022))
        except ValueError:
            rel_path = str(pdf)
        ...
        # Filtr roku — plik musi zawierać 2022 i NIE zawierać 2024/2025
        name_norm = name.lower()
        if "2022" not in name_norm:
            skip_other_year += 1
            continue
        ...
        if len(unique_matches) == 0 and client_strict and len(candidates) == 1:
            ...
            # Sanity check: plik musi być protokołem rocznym 2022
            if PROTOCOL_NUM_RE.match(name) or re.search(r"(?:^|\s|_)(\d+)_T_2022", name):
        ...
        if len(unique_matches) == 0:
            # Brak dopasowania
            mp = PROTOCOL_NUM_RE.match(name) or re.search(r"(?:^|\s|_)(\d+)_T_2022", name)
            no_ew_match.append({
                "filename": name,
                "rel_path": rel_path,
        ...
        proto_full = f"{proto_short}/T/2022" if proto_short else None
    ...
    # Output dryrun
    dryrun = {
        "_comment": "Phase A dryrun (lokalny Drive Stream scan) — protokoły roczne 2022",
        "scan_date": datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
        "drive_stream_root": str(GDRIVE_2022),
        "placeholders_2022_total": len(placeholders),
        "placeholders_2022_empty_before": sum(1 for p in placeholders if not p.get("protocol_pdf_url")),
    ...
    print("\n" + "=" * 78)
    print("PHASE A WYNIK 2022")
    print("=" * 78)
    print(f"  Placeholdery 2022 puste:         {dryrun['placeholders_2022_empty_before']}")
    print(f"  Plików PDF w GDrive 2022:        {len(all_pdfs)}")
    ...
    print(f"  Brakujące placeholdery 2022:     {len(missing_placeholders)}")
    ...
    for entry in matched:
        src = GDRIVE_2022 / entry["rel_path"]
        # Unique local_name = bezpieczna wersja source filename, prefix P_2022_
        base = safe_filename(entry["filename"])[:120]
        if not base.lower().endswith(".pdf"):
            base += ".pdf"
        local_name = f"P_2022_{base}"
        # Dedupe (na wypadek gdyby base już istniał)
        if local_name in used_local:
            local_name = f"P_2022_{used_local[local_name]+1}_{base}"
        ...
        manifest.append({
            ...
            "protocol_number": entry["protocol_number"] or f"AUTO-{entry['turbine_code']}/2022",
