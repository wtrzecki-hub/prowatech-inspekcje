"""Generate PDF report — turbiny bez protokołu rocznego 2021 w archiwum cyfrowym.

Czyta dane z scripts/output/manifest_2021_full_dryrun.json:
...
Output: Brakujace_protokoly_2021.pdf
"""
...
def header(canvas, doc):
    ...
    canvas.drawString(15 * mm, 10 * mm, f"Prowatech Inspekcje · Brakujące protokoły 2021 · {date.today().isoformat()}")
...
def main():
    output = PROJECT / "Brakujace_protokoly_2021.pdf"
    dryrun = json.loads((PROJECT / "scripts/output/manifest_2021_full_dryrun.json").read_text(encoding="utf-8"))
    ...
    doc = SimpleDocTemplate(
        ...
        title="Brakujące protokoły 2021 — Prowatech Inspekcje",
        author="ProWaTech Sp. z o.o.",
    )

    story = [
        Paragraph("Brakujące protokoły kontroli rocznej 2021", H1),
    ...
    total_placeholders = dryrun["placeholders_2021_empty_before"]
    summary_rows = [
        ["Łączna liczba placeholderów 2021 w bazie", str(total_placeholders)],  # 2021: wszystkie 139 puste przed sesją
        ["Pliki PDF znalezione w GDrive 2021", str(dryrun["total_pdf_files_scanned"])],
    ...
    legend = Table([
        [status_chip("no_file"), Paragraph(
            "Brak pliku PDF w GDrive 2021 dla tej turbiny. Operator albo nie wykonał "
            "kontroli rocznej w 2021, albo trzyma plik poza zasięgiem ProWaTech.", NORMAL)],
        [status_chip("multi"), Paragraph(
            "Plik istnieje w GDrive 2021, ale nazwa pasuje do kilku kandydatów turbin. "
            "Wymaga ręcznej decyzji który plik komu przypisać.", NORMAL)],
        [status_chip("no_match"), Paragraph(
            "Plik w GDrive 2021 ma nazwę protokołu, ale matcher nie znalazł żadnej "
    ...
    if no_file_clients:
        ...
        story.append(Paragraph(
            "Dla tych turbin nie znaleziono protokołu kontroli rocznej 2021 w GDrive 2021 "
            "(folder <font name='%s'>04 Inspekcje/2021</font>). Możliwe przyczyny: kontrola "
    ...
    if multi_rows:
        ...
        story.append(Paragraph(
            "Pliki PDF wyglądają jak protokoły roczne 2021, ale nazwa pasuje do kilku turbin "
            "jednocześnie (np. plik <i>EW 2 Solec 27-04-2021</i> może dotyczyć T329-Sumin lub "
    ...
    if nomatch_rows:
        ...
        story.append(Paragraph(
            "Pliki PDF z nazwy wyglądają jak protokoły 2021, ale matcher nie znalazł żadnej "
