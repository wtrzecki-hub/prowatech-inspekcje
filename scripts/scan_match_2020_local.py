#!/usr/bin/env python3
"""Phase A migracji 2020 — lokalny scan Drive Stream G: + match po ew_designation.

Strategia (jak Faza 15.G dla 2024):
  - Pobierz placeholdery 2020 z bazy (304 wpisy, 301 pustych)
  - Pobierz wszystkie 425 turbin z ew_designation, turbine_code, client_name, farm_name
  - Skanuj GDrive `04 Inspekcje/2020/` rekurencyjnie
  - Per plik: filtr non-protocol, wyciągnij ew_designation pasujący w nazwie, match per klient
  - Output: manifest_2020_full_dryrun.json + manifest_2020_full.json (gotowy do upload)
...
Uruchomienie:
  ...
  python scripts/scan_match_2020_local.py --copy     # + kopia plików do scripts/output/pdfs/
"""
...
GDRIVE_2020 = Path(r"G:\Dyski współdzielone\21 PROWATECH - INSPEKCJE\02 Archiwum_Prowatech\2020")
ROOT = Path(__file__).parent.parent
PDF_OUT_DIR = ROOT / "scripts/output/pdfs"
DRYRUN_PATH = ROOT / "scripts/output/manifest_2020_full_dryrun.json"
MANIFEST_PATH = ROOT / "scripts/output/manifest_2020_full.json"
...
NON_PROTOCOL_PATTERNS = [
    ...
    r"^FA_",  # 2020: faktury "FA_NN_2020_KLIENT.PDF"
    r"^F\s*VAT",  # 2020: faktury "F VAT NNN-MM-RR"
    r"\bfaktur",  # any "faktura" w nazwie
    r"upadkiem",  # 2020: "kontroli systemow ochrony przed upadkiem" (SOI nie PIIB)
    r"ewakuacyjn",  # 2020: "kontroli urzadzenia ewakuacyjnego" (SOI nie PIIB)
    r"(?:^|[^A-Za-z])PJ[_\s-]",  # 2020: "PJ_Badanie..." pomiary elektryczne
    r"badanie[\s_-]+napi",  # 2020: "Badanie_napięć_rażenia" pomiary (_ separator)
    r"napi[ęe]ć?[\s_-]+ra[zż]e",  # 2020: "napięć_rażenia"
    r"krokowych|dotykowych",  # 2020: pomiary napięć krokowych/dotykowych
]
NON_PROTOCOL_RE = re.compile("|".join(NON_PROTOCOL_PATTERNS), re.IGNORECASE)
PROTOCOL_NUM_RE = re.compile(r"^(\d+)_T_2020", re.IGNORECASE)
DATE_RE = re.compile(r"(\d{2})-(\d{2})-(2020)")
...
def fetch_db_data():
    ...
    while True:
        ...
        if len(res.data) < 1000:
            break
        offset += 1000
    print(f"[db] placeholdery 2020: {len(placeholders)} (puste: {sum(1 for p in placeholders if not p['protocol_pdf_url'])})")
...
def main():
    ...
    if not GDRIVE_2020.exists():
        print(f"[FATAL] Brak Drive Stream: {GDRIVE_2020}")
        sys.exit(2)
    ...
    # Skan plików
    all_pdfs = []
    for dirpath, dirnames, filenames in os.walk(str(GDRIVE_2020)):
        for fn in filenames:
            if fn.lower().endswith(".pdf"):
                all_pdfs.append(Path(dirpath) / fn)
    print(f"[scan] znaleziono {len(all_pdfs)} plików PDF w {GDRIVE_2020}")
    ...
    for pdf in all_pdfs:
        name = pdf.name
        try:
            rel_path = str(pdf.relative_to(GDRIVE_2020))
        except ValueError:
            rel_path = str(pdf)
        ...
        # Filtr roku — plik musi zawierać 2020 i NIE zawierać 2021-2025 (cross-year files w folderze 2020)
        name_norm = name.lower()
        if "2020" not in name_norm:
            skip_other_year += 1
            continue
        if any(y in name_norm for y in ("2021", "2022", "2023", "2024", "2025")):
            skip_other_year += 1
            continue
        ...
        # Klient hint z CAŁEJ rel_path — fuzzy search po słowach kluczowych klienta
        # 2020 ma multi-poziom hierarchii (EUROWIND/FW POŁUDNIE/DENMARK WIND/EW1),
        # więc samo top_folder=EUROWIND nie wystarczy. Szukamy tokenów po wszystkich folderach.
        # Zbieramy SCORE per klient (liczba matchujących tokenów), wybieramy MAX
        client_hint = normalize(rel_path.replace(os.sep, " "))
        candidates = []
        client_strict = False
        client_scores = []
        for client_name, t_list in by_client.items():
        ...
        for t in candidates:
            ...
            if ew_norm in name_norm_full:
                ...
                # WAŻNE 2020: szukaj WSZYSTKICH wystąpień, akceptuj jeśli choć jedno jest valid
                # (np. "EW2" w "Protokol 04_EW_2020 ... EW2 Olsza" pierwsze hit ma next='0' rejected,
                # ale drugie hit ma next='o' accepted)
                last_ch = ew_norm[-1]
                start = 0
                accepted = False
                while True:
                    idx = name_norm_full.find(ew_norm, start)
                    if idx == -1:
                        break
                    end = idx + len(ew_norm)
                    next_ch = name_norm_full[end] if end < len(name_norm_full) else ""
                    if not (last_ch.isdigit() and next_ch.isdigit()):
                        accepted = True
                        break
                    start = idx + 1
                if accepted:
        ...
        if len(unique_matches) == 0 and client_strict and len(candidates) == 1:
            ...
            # Sanity check: plik musi być protokołem rocznym 2020
            if PROTOCOL_NUM_RE.match(name) or re.search(r"(?:^|\s|_)(\d+)_(?:T|EW)_?-?2020", name) or re.match(r"^(\d+)\s+Protok[oó]", name):
        ...
        if len(unique_matches) == 0:
            ...
            for t in search_pool:
                ...
                # 2020: także po farm_name (bez prefixu "FW "/"EW ") — np. T177-Gościejewo
                # ma farm "FW ROGOŹNO", plik "EW Rogoźno 1" matchuje farmę nie lokalizację
                fn_raw = t.get("_farm_name") or ""
                fn_clean = re.sub(r"^(?:FW|EW|WS)\s+", "", fn_raw, flags=re.IGNORECASE).strip()
                if fn_clean and len(fn_clean) >= 4:
                    fn_norm = normalize(fn_clean)
                    if fn_norm and fn_norm in name_norm_full:
            ...
            if len(uniq) == 1:
                ...
                if ew_numbers_in_name:
                    ...
                    elif len(digit_filtered) == 0:
                        # 2020: cyfry w nazwie nie pasują do żadnej turbiny w pool
                        # (np. "WTG 17 Kołaczkowo" gdy klient ma WTG 01..15) → drop
                        digit_filtered_empty = True
                if digit_filtered_empty:
                    mp = PROTOCOL_NUM_RE.match(name) or re.search(r"(?:^|\s|_)(\d+)_(?:T|EW)_?-?2020", name) or re.match(r"^(\d+)\s+Protok[oó]", name)
                    no_ew_match.append({
                        "filename": name,
                        "rel_path": rel_path,
                        ...
                        "drop_reason": "digit_outside_pool",
                    })
                    continue
                if len(unique_matches) == 0:
                    multi_match.append({
        ...
        proto_full = f"{proto_short}/T/2020" if proto_short else None
    ...
    # Output dryrun
    dryrun = {
        "_comment": "Phase A dryrun (lokalny Drive Stream scan) — protokoły roczne 2020",
        "scan_date": datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
        "drive_stream_root": str(GDRIVE_2020),
        "placeholders_2020_total": len(placeholders),
        "placeholders_2020_empty_before": sum(1 for p in placeholders if not p.get("protocol_pdf_url")),
    ...
    print("\n" + "=" * 78)
    print("PHASE A WYNIK 2020")
    print("=" * 78)
    print(f"  Placeholdery 2020 puste:         {dryrun['placeholders_2020_empty_before']}")
    print(f"  Plików PDF w GDrive 2020:        {len(all_pdfs)}")
    ...
    print(f"  Brakujące placeholdery 2020:     {len(missing_placeholders)}")
    ...
    for entry in matched:
        src = GDRIVE_2020 / entry["rel_path"]
        # Unique local_name = bezpieczna wersja source filename, prefix P_2020_
        base = safe_filename(entry["filename"])[:120]
        if not base.lower().endswith(".pdf"):
            base += ".pdf"
        local_name = f"P_2020_{base}"
        # Dedupe (na wypadek gdyby base już istniał)
        if local_name in used_local:
            local_name = f"P_2020_{used_local[local_name]+1}_{base}"
        ...
        manifest.append({
            ...
            "protocol_number": entry["protocol_number"] or f"AUTO-{entry['turbine_code']}/2020",
