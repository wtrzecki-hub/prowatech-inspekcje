"""Generate PDF report — turbiny bez protokołu rocznego 2022 w archiwum cyfrowym.

Czyta dane z scripts/output/manifest_2022_full_dryrun.json:
...
Output: Brakujace_protokoly_2022.pdf
"""
...
def header(canvas, doc):
    ...
    canvas.drawString(15 * mm, 10 * mm, f"Prowatech Inspekcje · Brakujące protokoły 2022 · {date.today().isoformat()}")
...
def main():
    output = PROJECT / "Brakujace_protokoly_2022.pdf"
    dryrun = json.loads((PROJECT / "scripts/output/manifest_2022_full_dryrun.json").read_text(encoding="utf-8"))
    ...
    doc = SimpleDocTemplate(
        ...
        title="Brakujące protokoły 2022 — Prowatech Inspekcje",
        author="ProWaTech Sp. z o.o.",
    )

    story = [
        Paragraph("Brakujące protokoły kontroli rocznej 2022", H1),
    ...
    total_placeholders = dryrun["placeholders_2022_empty_before"]
    summary_rows = [
        ["Łączna liczba placeholderów 2022 w bazie", str(total_placeholders + 3)],  # +3 bo było 3 z plikiem przed sesją
        ["Pliki PDF znalezione w GDrive 2022", str(dryrun["total_pdf_files_scanned"])],
    ...
    legend = Table([
        [status_chip("no_file"), Paragraph(
            "Brak pliku PDF w GDrive 2022 dla tej turbiny. Operator albo nie wykonał "
            "kontroli rocznej w 2022, albo trzyma plik poza zasięgiem ProWaTech.", NORMAL)],
        [status_chip("multi"), Paragraph(
            "Plik istnieje w GDrive 2022, ale nazwa pasuje do kilku kandydatów turbin. "
            "Wymaga ręcznej decyzji który plik komu przypisać.", NORMAL)],
        [status_chip("no_match"), Paragraph(
            "Plik w GDrive 2022 ma nazwę protokołu, ale matcher nie znalazł żadnej "
    ...
    if no_file_clients:
        ...
        story.append(Paragraph(
            "Dla tych turbin nie znaleziono protokołu kontroli rocznej 2022 w GDrive 2022 "
            "(folder <font name='%s'>04 Inspekcje/2022</font>). Możliwe przyczyny: kontrola "
    ...
    if multi_rows:
        ...
        story.append(Paragraph(
            "Pliki PDF wyglądają jak protokoły roczne 2022, ale nazwa pasuje do kilku turbin "
            "jednocześnie (np. plik <i>EW 2 Solec 27-04-2022</i> może dotyczyć T329-Sumin lub "
    ...
    if nomatch_rows:
        ...
        story.append(Paragraph(
            "Pliki PDF z nazwy wyglądają jak protokoły 2022, ale matcher nie znalazł żadnej "
