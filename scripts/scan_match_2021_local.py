#!/usr/bin/env python3
"""Phase A migracji 2021 — lokalny scan Drive Stream G: + match po ew_designation.

Strategia (jak Faza 15.G dla 2024):
  - Pobierz placeholdery 2021 z bazy (304 wpisy, 301 pustych)
  - Pobierz wszystkie 425 turbin z ew_designation, turbine_code, client_name, farm_name
  - Skanuj GDrive `04 Inspekcje/2021/` rekurencyjnie
  - Per plik: filtr non-protocol, wyciągnij ew_designation pasujący w nazwie, match per klient
  - Output: manifest_2021_full_dryrun.json + manifest_2021_full.json (gotowy do upload)
...
Uruchomienie:
  ...
  python scripts/scan_match_2021_local.py --copy     # + kopia plików do scripts/output/pdfs/
"""
...
GDRIVE_2021 = Path(r"G:\Dyski współdzielone\21 PROWATECH - INSPEKCJE\02 Archiwum_Prowatech\2021")
ROOT = Path(__file__).parent.parent
PDF_OUT_DIR = ROOT / "scripts/output/pdfs"
DRYRUN_PATH = ROOT / "scripts/output/manifest_2021_full_dryrun.json"
MANIFEST_PATH = ROOT / "scripts/output/manifest_2021_full.json"
...
NON_PROTOCOL_PATTERNS = [
    ...
    r"^FA_",  # 2021: faktury "FA_NN_2021_KLIENT.PDF"
    r"^F\s*VAT",  # 2021: faktury "F VAT NNN-MM-RR"
    r"\bfaktur",  # any "faktura" w nazwie
    r"upadkiem",  # 2021: "kontroli systemow ochrony przed upadkiem" (SOI nie PIIB)
    r"ewakuacyjn",  # 2021: "kontroli urzadzenia ewakuacyjnego" (SOI nie PIIB)
]
NON_PROTOCOL_RE = re.compile("|".join(NON_PROTOCOL_PATTERNS), re.IGNORECASE)
PROTOCOL_NUM_RE = re.compile(r"^(\d+)_T_2021", re.IGNORECASE)
DATE_RE = re.compile(r"(\d{2})-(\d{2})-(2021)")
...
def fetch_db_data():
    ...
    while True:
        ...
        if len(res.data) < 1000:
            break
        offset += 1000
    print(f"[db] placeholdery 2021: {len(placeholders)} (puste: {sum(1 for p in placeholders if not p['protocol_pdf_url'])})")
...
def main():
    ...
    if not GDRIVE_2021.exists():
        print(f"[FATAL] Brak Drive Stream: {GDRIVE_2021}")
        sys.exit(2)
    ...
    # Skan plików
    all_pdfs = []
    for dirpath, dirnames, filenames in os.walk(str(GDRIVE_2021)):
        for fn in filenames:
            if fn.lower().endswith(".pdf"):
                all_pdfs.append(Path(dirpath) / fn)
    print(f"[scan] znaleziono {len(all_pdfs)} plików PDF w {GDRIVE_2021}")
    ...
    for pdf in all_pdfs:
        name = pdf.name
        try:
            rel_path = str(pdf.relative_to(GDRIVE_2021))
        except ValueError:
            rel_path = str(pdf)
        ...
        # Filtr roku — plik musi zawierać 2021 i NIE zawierać 2024/2025
        name_norm = name.lower()
        if "2021" not in name_norm:
            skip_other_year += 1
            continue
        ...
        # 2021 ma multi-poziom hierarchii (EUROWIND/FW POŁUDNIE/DENMARK WIND/EW1),
        ...
        for t in candidates:
            ...
            if ew_norm in name_norm_full:
                ...
                # WAŻNE 2021: szukaj WSZYSTKICH wystąpień, akceptuj jeśli choć jedno jest valid
                # (np. "EW2" w "Protokol 04_EW_2021 ... EW2 Olsza" pierwsze hit ma next='0' rejected,
        ...
        if len(unique_matches) == 0 and client_strict and len(candidates) == 1:
            ...
            # Sanity check: plik musi być protokołem rocznym 2021
            if PROTOCOL_NUM_RE.match(name) or re.search(r"(?:^|\s|_)(\d+)_(?:T|EW)_?-?2021", name):
        ...
        if len(unique_matches) == 0:
            ...
            for t in search_pool:
                ...
                # 2021: także po farm_name (bez prefixu "FW "/"EW ") — np. T177-Gościejewo
            ...
            if len(uniq) == 1:
                ...
                if ew_numbers_in_name:
                    ...
                    if len(digit_filtered) == 1:
                        ...
                        # 2021: cyfry w nazwie nie pasują do żadnej turbiny w pool
                        # (np. "WTG 17 Kołaczkowo" gdy klient ma WTG 01..15) → drop
                        digit_filtered_empty = True
                if digit_filtered_empty:
                    mp = PROTOCOL_NUM_RE.match(name) or re.search(r"(?:^|\s|_)(\d+)_(?:T|EW)_?-?2021", name)
                    no_ew_match.append({
                        "filename": name,
                        "rel_path": rel_path,
        ...
        proto_full = f"{proto_short}/T/2021" if proto_short else None
    ...
    # Output dryrun
    dryrun = {
        "_comment": "Phase A dryrun (lokalny Drive Stream scan) — protokoły roczne 2021",
        "scan_date": datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
        "drive_stream_root": str(GDRIVE_2021),
        "placeholders_2021_total": len(placeholders),
        "placeholders_2021_empty_before": sum(1 for p in placeholders if not p.get("protocol_pdf_url")),
    ...
    print("\n" + "=" * 78)
    print("PHASE A WYNIK 2021")
    print("=" * 78)
    print(f"  Placeholdery 2021 puste:         {dryrun['placeholders_2021_empty_before']}")
    print(f"  Plików PDF w GDrive 2021:        {len(all_pdfs)}")
    ...
    print(f"  Brakujące placeholdery 2021:     {len(missing_placeholders)}")
    ...
    for entry in matched:
        src = GDRIVE_2021 / entry["rel_path"]
        # Unique local_name = bezpieczna wersja source filename, prefix P_2021_
        base = safe_filename(entry["filename"])[:120]
        if not base.lower().endswith(".pdf"):
            base += ".pdf"
        local_name = f"P_2021_{base}"
        # Dedupe (na wypadek gdyby base już istniał)
        if local_name in used_local:
            local_name = f"P_2021_{used_local[local_name]+1}_{base}"
        ...
        manifest.append({
            ...
            "protocol_number": entry["protocol_number"] or f"AUTO-{entry['turbine_code']}/2021",
