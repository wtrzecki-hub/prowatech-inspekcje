...
## 4. Metryczka — duplikacja słowa „Okazano"
...
## 5. Generator DOCX — błędni wykonawcy „Andrzej i Tomek"
...
## 6. Generator DOCX — pusta sekcja „Załączniki" rendererowana z 6 placeholder-punktami
...
## 7. Generator DOCX/PDF — sekcja III. USTALENIA nie zawiera zaleceń z poprzedniego roku ani opisów elementów
...
## 8. Propagacja niewykonanych zaleceń z prev_rec → element + zdjęcia (auto-defect)

**Status:** Faza A wykonana 2026-05-13. Fazy B + C → osobne tematy. Źródło: Waldek 2026-05-12 (po deploy PR #40).

**Wykonane w Fazie A:**
- Migracja `2026-05-13_inspection_photos_source_rec.sql` (kolumna FK do prev_recs)
- `ensureCarryToElement` w `previous-recommendations-table.tsx` — match po `name_pl OR name_short`, append do `inspection_elements.recommendations` z prefiksem „Z poprzedniej kontroli", carry zdjęć z dedupem po `source_recommendation_id`
- Reverse: `revertElementCarry` usuwa zdjęcia przy odznaczeniu „Nie wykonano" (tekstu nie cofa)
- Faza B (picker `defect_library`) była już zaimplementowana wcześniej w `element-card.tsx` (commit `3738493 feat(inspekcje): biblioteka usterek w karcie elementu`)
- Faza C (auto-suggest fuzzy z `defect_library` po wpisaniu w `recommendations`) → osobny PR po stabilizacji A.

**Symptom Waldka:** „Ustalenia i wnioski — jak przepisujemy niewykonane zalecenia z poprzedniej kontroli, powinny być dodane do elementów wraz ze zdjęciem dodanym do zaleceń przepisanych z poprzedniej kontroli."

**Cel:** Auto-propagacja przy carry-over „Nie wykonano":
- `prev_rec` (z `element_name` set) → `repair_scope_items` (carry działa od PR #32)
- Dodatkowo: tekst zalecenia i zdjęcia powinny też pojawić się w `inspection_elements` dla danego elementu (kolumna „Zalecenia" w sekcji III tabeli ustaleń)

**Co już jest w bazie:**
- `defect_library` (138 wpisów) — `element_section_code`, `element_number`, `description_template`, `recommendation_template`, `typical_rating`, `typical_urgency`
- `inspection_photos.element_id` — galeria per element
- `recommendation_photos` (parent_type='previous_recommendation' / 'repair_scope_item') — galeria per zalecenie

**Propozycja podziału na fazy (z wcześniejszej rozmowy 2026-05-12):**

**Faza A** — manualna propagacja prev_rec → element:
- Przy zaznaczeniu „Nie wykonano" w prev_rec z `element_name` matchującym definition.name_pl:
  - Znajdź `inspection_elements.element_definition_id` po `name_pl` (fuzzy match)
  - Append `recommendation_text` do `inspection_elements.recommendations` (z prefiksem „Z poprzedniej kontroli: ...")
  - Skopiuj zdjęcia z `recommendation_photos` → `inspection_photos` z `element_id`

**Faza B** — auto-defect z `defect_library`:
- Po wybraniu elementu, jeśli tekst recommendations match'uje `defect_library.recommendation_template` (fuzzy >0.7, po `element_number`) → button „Dodaj usterkę z bazy" w element-card.tsx
- Klik wypełnia `notes`, `recommendations`, `condition_rating`

**Faza C** — pełna automatyzacja:
- Bez button-a, propagacja przy carry-over + toast „Wciągnięto X zaleceń do elementów Y"

**Decyzje do podjęcia w sesji:**
- Czy `element_name` w prev_rec ma być **dokładnie** taki sam jak `element_definitions.name_pl` (wymusić Select wbudowany z PR #37 ma już datalist, ale freetext) czy fuzzy match z tolerancją?
- Co z numeracją zdjęć — duplikujemy wskaźnik R2 między galeriami czy migrujemy/przenosimy?
- Tylko Faza A czy od razu plan na B i C?
...
- [x] **Freeze podpisanych protokołów + Faza A propagacji prev_rec → element** — **2026-05-13** — TBD
  - Zasada: `inspections.status = 'signed'` → read-only (UI pointer-events:none + backend guards). Helper `isInspectionLocked` w `@/lib/inspections/locked.ts`. Banner na karcie inspekcji. Defense in depth w `previous-recommendations-table.tsx`, `inspection-metadata-piib.tsx`, `[id]/page.tsx.handleInspectionChange`. **Follow-up:** defense in depth w pozostałych ~10 komponentach inspekcji (na razie chronione tylko UI-level).
  - Faza A: migracja `inspection_photos.source_recommendation_id` + `ensureCarryToElement` (match `name_pl OR name_short`) + reverse zdjęć przy odznaczeniu „Nie wykonano".

- [x] **Zamknięte werbalnie 2026-05-12 (sesja popołudniowa) — bez PR-a:**
  - „Wykonawca kontroli nie zaznacza się" — non-issue, brak repro od Artura. SQL na bazie pokazuje że ostatnie inspekcje mają poprawne wpisy w `inspection_inspectors`, brak ERROR-ów w postgres logach. Powiązany fix legacy „Andrzej i Tomek" w generatorze → PR [#34](https://github.com/wtrzecki-hub/prowatech-inspekcje/pull/34).
  - Pytanie o widoczność „Następna kontrola 5-letnia / elektryczna" na karcie turbiny — decyzja Waldka.
  - DOCX „chyba lepiej, żeby było odwrotnie" — Waldek wyjaśnił z Arturem, non-issue.
- [x] **„Szkic" w nagłówku PROTOKÓŁ NR + adres siedziby właściciela w metryczce** — **2026-05-12** — [#40](https://github.com/wtrzecki-hub/prowatech-inspekcje/pull/40)
- [x] **Odrzuć „-" jako prawidłowy license_number + odzyskaj utracone commity #34 (sygnariusz/branżowy + pieczątka 5cm)** — **2026-05-12** — [#39](https://github.com/wtrzecki-hub/prowatech-inspekcje/pull/39)
- [x] **Nazwa pliku PDF/DOCX: „Szkic" zamiast UUID + brak duplikacji „Kamlarki Kamlarki"** — **2026-05-12** — [#38](https://github.com/wtrzecki-hub/prowatech-inspekcje/pull/38)
- [x] **Pole „Element / lokalizacja" — autocomplete z listy PIIB + propagacja prev_rec → scope** — **2026-05-12** — [#37](https://github.com/wtrzecki-hub/prowatech-inspekcje/pull/37)
