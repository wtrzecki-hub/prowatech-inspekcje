...
**Faza 15.G dla 2024 DOMKNIĘTA + EDP odzyskane (2026-04-28 wieczór, 2 tury) — 210 plików wgranych, 2024 archiwum PIIB w 62.7% (237/378).** Pipeline zwalidowany na full bulk + dochodzeniowa runda dla zewnętrznego wykonawcy (GP EARTH dla EDP). Pozostaje: **2023 (303 placeholderów), 2022 (210), 2021 (139), 2020 (99), plus 141 niewypełnionych 2024 (z tego ~10 hard cases zidentyfikowanych, ~131 do dochodzenia w GDrive 2025)**. Razem ~752 placeholderów do uzupełnienia w kolejnych sesjach. **Plan dla następnej sesji**:

1. **Wczytaj PROGRESS.md, zwłaszcza wpis "Ostatnia aktualizacja 2026-04-28 (wieczór)"** — opisuje pipeline 2-fazowy (Phase A dryrun + Phase B stream-mode) i zasadę "bulk >100 → split na ≤85 per subagent".
2. **Załaduj ToolSearch:** GDrive (`search_files`, `download_file_content`, `get_file_metadata`), Supabase (`execute_sql`), `mcp__workspace__bash`, `Agent`/`TaskCreate`.
3. **Foldery GDrive lat:** 2022 (`1ADSFC15Qw3kR5CQY4dNca28tC9P3-MVo`), 2023 (`1wgKj0uG5e_HbDABCPg3xu-uCpn09W5NT`), 2024 (`1LzagkKOumY672gmy17t-Rh3Ec_Rac1RR`), 2025 (`163SeIT7Ov-E7MoylXmAi2vf9FrUQ0r-r`). Każdy ~46-50 folderów klienta z PDF-ami protokołów bezpośrednio w środku (subfoldery turbin tylko Zdjęcia/).
4. **Workflow per rok (zwalidowany na 2024):**
   - **Faza A (1 subagent)**: scan 46 folderów → match po `ew_designation` → manifest dryrun (matched + unmatched). Brief: `scripts/output/agent_brief_phase_a_dryrun.md` (do skopiowania per-rok zamiana ID folderu).
   - **Faza B (2 subagenty po ~85 plików, sekwencyjnie)**: stream-mode download → manifest_full_partN.json. Brief: `scripts/output/agent_brief_phase_b_download.md`.
   - **Merge** parentem: `manifest_<rok>_full.json` w formacie `upload_batch.py`.
   - **Upload lokalnie** Waldek: `python scripts/upload_batch.py scripts/output/manifest_<rok>_full.json`.
5. **Match po `ew_designation`** — wszystkie 425 turbin mają wartość. Cross-client OK (deterministyczny match niezależnie od folderu klienta GDrive).
6. **Dochodzenie 2024 placeholderów (184)**: część fizycznie w folderze GDrive 2025 (subagent zauważył EDP — protokoły 2024 z mod-time 2025-03-19). Skan folderu 2025 pod kątem regex parsera + `year=2024` z nazwy. Bonus: 10 hard cases unmatched (3× CIME WIND VESTAS Krzanowice, 4× GPO substacje, 2× FWZ Vensys/Vestas, 1× WS Wind Park VI Bławaty/Brzeźno) — do ręcznej decyzji.

**Stan obecny po 2026-04-28 (wieczór, runda 2):**
- `historical_protocols`: **237 wpisów wypełnionych 2024** (62.7%, +210 dziś wieczorem w 2 turach) + 1 z 2023 + 941 placeholderów do wypełnienia (141 z 2024 + 303 z 2023 + 210 z 2022 + 139 z 2021 + 99 z 2020). Z 141 unfilled 2024: ~10 hard cases + ~131 fizycznie brakujących w folderze GDrive 2024.
- Wszystkie 425 turbin mają `ew_designation` (z porannej sesji).
- Pipeline zwalidowany end-to-end na **210 plikach** (167 standard + 43 EDP) → 0 błędów łącznie.
- Workspace `scripts/output/pdfs/` ma 238 plików (167 standard + 43 EDP + 28 z porannej). ~687 MB, można czyścić po wszystkim.
- Hostowe `.txt` z `download_file_content` cleanowane: `cleanup_txt_hosts.txt` (B1+B2, 82 ścieżek) + `edp_txt_cleanup.txt` (43 ścieżek). Waldek wykonał oba `Get-Content ... | Remove-Item -Force`.
- **EDP `inspection_date = NULL`** w 43 wpisach — opcjonalny SQL UPDATE na `2024-03-07` jako proxy date jeśli chcemy mieć daty w UI sortowaniu / hero. Na razie odłożone — Waldek decyduje.
...
- UI `historical-protocols-tab.tsx` pokazuje dla placeholderów (z `protocol_pdf_url=NULL`) przycisk "PDF" jakby plik był dostępny — klik nic nie zrobi (404). Powinno być `disabled` lub "—" dla placeholderów. Sweep ~10 min, niski priorytet.
