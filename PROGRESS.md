...
_Ostatnia aktualizacja: 2026-04-26 — **Faza 16.1 (Naprawa flow `completed → signed`) DONE + zweryfikowane na prod.** DB trigger `tr_inspections_auto_sign` + pole `owner_signature_date` w UI inspekcji. Inspektor wpisuje obie daty podpisu, trigger automatycznie awansuje status na `signed`, klient widzi protokół w portalu. Migracja w `migrations/2026-04-26_auto_sign_inspection_trigger.sql`. Wątek z Fazy 16 zamknięty._
